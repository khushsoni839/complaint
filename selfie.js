  ...
  app.get("/api/selfies", async (req, res) => {
    try {
      ...
      const data = await Selfie.find(query)
        .select(
  "username name address From To Location MobileNo epnbd inv bat pan Inverter Battery Panel Mode km Amount remarks date location timestamp visitOutcome morningMeter eveningMeter"
)
        .sort({ timestamp: -1 })
        .lean();
  ...
  // app.post("/api/selfie", upload.single("image"), async (req, res) => {
  //   try {
